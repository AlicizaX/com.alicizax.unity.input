...
public static partial class UXInput
{
    ...
    public static class Reader
    {
        ...
        private static InputReadStateMap _pressedKeys;
        private static InputReadStateMap _toggledKeys;
        private static CompositePartControlMap _compositePartControls;
        private static bool _bindingChangeListenerRegistered;

        /// <summary>
        /// Reader 诊断读取类型。
        /// </summary>
        public enum ReadTraceKind
        {
            Value,
            ValueOnce,
            Button,
            ...
            PressedToggle,
            CompositePartButton,
            CompositePartButtonOnce,
            CompositePartButtonToggle
        }

        /// <summary>
        /// Reader 诊断读取记录。
        /// </summary>
        public readonly struct ReadTrace
        {
            public readonly ReadTraceKind Kind;
        ...
        /// 在一次触发、切换或一次值读取发生时触发。
        /// </summary>
        public static event Action<ReadTrace> OnRead;

        /// <summary>
        /// 在连续读取 API 读到有效按下状态时触发，可能高频触发。
        /// </summary>
        public static event Action<ReadTrace> OnContinuousRead;

        private static void EnsureBindingChangeListener()
        {
            if (_bindingChangeListenerRegistered)
            {
                return;
            }

            Rebind.OnBindingsChanged -= ClearBindingCaches;
            Rebind.OnBindingsChanged += ClearBindingCaches;
            _bindingChangeListenerRegistered = true;
        }
        ...
        {
            return string.IsNullOrWhiteSpace(actionName) ? null : InputActionProvider.ResolveAction(actionName);
        }
        ...
        public static bool TryReadValue<T>(InputAction action, out T value) where T : struct
        {
            if (action != null && action.IsPressed())
            {
                ...
                RaiseContinuousReadTrace(ReadTraceKind.Value, action, null, null, 0, null, true);
                return true;
            }
        ...
        {
            bool isPressed = ReadButtonInternal(action);
            if (isPressed)
            {
                RaiseContinuousReadTrace(ReadTraceKind.Button, action, null, null, 0, null, true);
            }
        ...
        {
            bool isPressed = ReadPressedInternal(action);
            if (isPressed)
            {
                RaiseContinuousReadTrace(ReadTraceKind.Pressed, action, null, null, 0, null, true);
            }
        ...
        {
            return ReadButtonOnceInternal(action, null, null, 0, key, ReadPressedInternal(action), ReadTraceKind.PressedOnce);
        }
        ...
        {
            InputAction action = ResolveAction(actionName);
            return ReadButtonOnceInternal(action, actionName, null, 0, key, ReadPressedInternal(action), ReadTraceKind.PressedOnce);
        }
        ...
        {
            return ReadButtonToggleInternal(action, null, null, 0, key, ReadPressedInternal(action), ReadTraceKind.PressedToggle);
        }
        ...
        {
            InputAction action = ResolveAction(actionName);
            return ReadButtonToggleInternal(action, actionName, null, 0, key, ReadPressedInternal(action), ReadTraceKind.PressedToggle);
        }
        ...
        {
            return ReadButtonOnceInternal(action, null, null, 0, key, ReadButtonInternal(action), ReadTraceKind.ButtonOnce);
        }
        ...
        {
            InputAction action = ResolveAction(actionName);
            return ReadButtonOnceInternal(action, actionName, null, 0, key, ReadButtonInternal(action), ReadTraceKind.ButtonOnce);
        }
        ...
        {
            return ReadButtonToggleInternal(action, null, null, 0, key, ReadButtonInternal(action), ReadTraceKind.ButtonToggle);
        }
        ...
        {
            InputAction action = ResolveAction(actionName);
            return ReadButtonToggleInternal(action, actionName, null, 0, key, ReadButtonInternal(action), ReadTraceKind.ButtonToggle);
        }
        ...
        {
            bool isPressed = ReadCompositePartButtonInternal(action, compositePartName);
            if (isPressed)
            {
                RaiseContinuousReadTrace(
                    ReadTraceKind.CompositePartButton,
                    action,
                    null,
                    compositePartName,
                    0,
        ...
        public static bool ReadCompositePartButtonOnce(int ownerId, InputAction action, string compositePartName)
        {
            return ReadButtonOnceInternal(
                ...
                ownerId,
                null,
                ReadCompositePartButtonInternal(action, compositePartName),
                ReadTraceKind.CompositePartButtonOnce);
        }
        ...
        public static bool ReadCompositePartButtonToggle(int ownerId, InputAction action, string compositePartName)
        {
            return ReadButtonToggleInternal(
                ...
                null,
                ReadCompositePartButtonInternal(action, compositePartName),
                ReadTraceKind.CompositePartButtonToggle);
        }
        ...
        {
            InputReadStateMap toggledKeys = _toggledKeys;
            if (toggledKeys == null)
            {
                return;
            }
        ...
        {
            InputReadStateMap toggledKeys = _toggledKeys;
            if (action == null || toggledKeys == null)
            {
                return;
            }
            ...
            out T value) where T : struct
        {
            ...
            if (action != null && action.IsPressed())
            {
                if (GetPressedKeys().Add(in actionId, actionKey, compositePartName, ownerId, ownerKey))
                {
                    value = action.ReadValue<T>();
                    RaiseReadTrace(ReadTraceKind.ValueOnce, action, actionName, compositePartName, ownerId, ownerKey, true);
                    return true;
                }
            }
            else
            {
                _pressedKeys?.Remove(in actionId, actionKey, compositePartName, ownerId, ownerKey);
            }
            ...
            bool emitTrace = true)
        {
            ...
            if (isPressed)
            {
                if (GetPressedKeys().Add(in actionId, actionKey, compositePartName, ownerId, ownerKey))
                {
                    if (emitTrace)
                    {
                        RaiseReadTrace(traceKind, action, actionName, compositePartName, ownerId, ownerKey, true);
                    }
            ...
            ReadTraceKind traceKind)
        {
                    ...
                    false))
            {
                ...
                bool boolValue = toggledKeys.Contains(in actionId, actionKey, compositePartName, ownerId, ownerKey);
                RaiseReadTrace(traceKind, action, actionName, compositePartName, ownerId, ownerKey, boolValue);
                return boolValue;
            }

            return toggledKeys != null && toggledKeys.Contains(in actionId, actionKey, compositePartName, ownerId, ownerKey);
        }

        private static bool ReadPressedInternal(InputAction action)
        {
            return action != null && action.IsPressed();
        }

        private static bool ReadButtonInternal(InputAction action)
        {
            return action != null && action.type == InputActionType.Button && action.IsPressed();
        }

        private static InputReadStateMap GetPressedKeys()
        {
            return _pressedKeys ??= new InputReadStateMap(InitialStateCapacity);
        }

        private static InputReadStateMap GetToggledKeys()
        {
            return _toggledKeys ??= new InputReadStateMap(InitialStateCapacity);
        }

        private static CompositePartControlMap GetCompositePartControls()
        {
            return _compositePartControls ??= new CompositePartControlMap(32);
        }

        private static bool ReadCompositePartButtonInternal(InputAction action, string compositePartName)
        {
            ...
            EnsureBindingChangeListener();
            CompositePartControlMap compositePartControls = GetCompositePartControls();
        ...
        private static void RaiseReadTrace(
            ReadTraceKind kind,
            InputAction action,
            string actionName,
            string compositePartName,
            ...
            bool boolValue)
        {
            Action<ReadTrace> handler = OnRead;
            if (handler == null)
            {
                return;
            }

            handler(new ReadTrace(kind, action, actionName, compositePartName, ownerId, ownerKey, boolValue));
        }

        private static void RaiseContinuousReadTrace(
            ReadTraceKind kind,
            InputAction action,
            string actionName,
            string compositePartName,
            ...
            bool boolValue)
        {
            Action<ReadTrace> handler = OnContinuousRead;
            if (handler == null)
            {
                return;
            }
        ...
        private sealed class InputReadStateMap
        {
            ...
            public void RemoveByActionName(string actionName)
            {
                ...
                for (int i = 0; i < _occupied.Length;)
                {
                    if (_occupied[i] && string.Equals(_actionNames[i], normalizedActionName, StringComparison.Ordinal))
                    {
                        RemoveAt(i);
                        continue;
                    }
                ...
                string ownerKey)
            {
                return _actionIds[slot].Equals(actionId)
                       ...
                       && string.Equals(_actionNames[slot], Normalize(actionName), StringComparison.Ordinal)
                       && string.Equals(_compositePartNames[slot], Normalize(compositePartName), StringComparison.Ordinal)
                       && string.Equals(_ownerKeys[slot], Normalize(ownerKey), StringComparison.Ordinal);
            }
            ...
            public bool TryGet(InputAction action, string partName, out InputControl control)
            {
                ...
                do
                {
                    ...
                    if (_hashes[slot] == hash
                        ...
                        && string.Equals(_partNames[slot], normalizedPartName, StringComparison.Ordinal))
                    {
            ...
            private void SetInternal(int hash, int actionHash, in Guid actionId, string partName, InputControl control)
            {
                ...
                while (_occupied[slot])
                {
                    if (_hashes[slot] == hash
                        && _actionHashes[slot] == actionHash
                        && _actionIds[slot].Equals(actionId)
                        && string.Equals(_partNames[slot], partName, StringComparison.Ordinal))
                    {
        ...
        private static int BuildCompositeHash(int actionHash, in Guid actionId, string partName)
        {
            unchecked
            {
                ...
                hash = (hash * 31) + actionId.GetHashCode();
                hash = (hash * 31) + StableHash(partName);
                return hash == 0 ? 1 : hash;
            }
        }
